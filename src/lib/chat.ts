...
class ChatService {
  ...
  async performOcr(imageData?: string, imageUrl?: string): Promise<OcrResponse> {
    try {
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to perform OCR' };
    }
  }
  async sendMessage(
    message: string,
