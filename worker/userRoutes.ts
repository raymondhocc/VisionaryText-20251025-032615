...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
     ...
     * POST /api/ocr
     * Body: { imageData?: string (base64), imageUrl?: string }
     */
    app.post('/api/ocr', async (c) => {
        try {
            const { imageData, imageUrl } = await c.req.json();
            if (!imageData && !imageUrl) {
                return c.json({ success: false, error: 'Image data or URL is required.' }, { status: 400 });
            }
            const result = await executeTool('perform_ocr', { imageData, imageUrl });
            if ('error' in result) {
                return c.json({ success: false, error: result.error }, { status: 500 });
            }
            if ('content' in result) {
                 return c.json({ success: true, data: { text: result.content } });
            }
            return c.json({ success: false, error: 'Unexpected OCR result format' }, { status: 500 });
        } catch (error) {
            console.error('OCR processing error:', error);
            return c.json({ success: false, error: 'Failed to process image for OCR.' }, { status: 500 });
        }
    });
    /**
